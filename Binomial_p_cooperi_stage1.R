...
data_summary(s2, varname="Success", groupnames=c("fpop"))
...
#estimate(-0.08043) and P value (0.459) no sign difference

cld(lsmeans(M5, ~ftemp), Letters=letters)

################ REMOVE POP #######################################
#' We would like to remove pop because pop does not have any significant 
#' effect on seed germination (stage 1)

# Section 13: Model formulation----
...
#' Execute the binomial GLMM using glmmTMB
M1a <- glmmTMB(cbind(count, Failure) ~ ftemp*ffun,
              data = s2,
              family = binomial)
...
# Section 15: Check overdispersion of the Binomial GLMM----

#' Get the Pearson residuals
E1 <- resid(M1a, type = "pearson")

#' Get the sample size and the number of parameters.
N    <- nrow(s2)
Npar <- length(fixef(M1a)$cond) + 1 #' The '+1' is for the sigma
...
# Section 16 Model Validation binomial GLMM----


#* Subsection 16.1: Plot residuals vs fitted values----
...
#' What does DHARMa tell us?
#' Get scaled quantile residuals.
E1Binqr <- simulateResiduals(fittedModel = M1a, plot = FALSE)
#' warning message that we have a deviation from uniformity
...
plotResiduals(E1Binqr, form = s2$ffun, xlab = "Rank-transformed fun") 
plotResiduals(E1Binqr, form = s2$ftemp, xlab = "Rank-transformed temp") 
#' Trouble for the first and last? There is certainly a non-linear pattern
#' in the residuals with fungi, population, and temp.
#' some within-group residual distributions that are not uniform
...
# Section 17: Visualise the results of the binomial GLMM----


#* Subsection 17.1: Visualise the model fit----
...
MyData <- expand.grid(ffun = levels(s2$ffun), 
                      ftemp = levels(s2$ftemp))
MyData
...
#' SE of fitted values are given by the square root of
#' the diagonal elements of: X * cov(betas) * t(X)
MyData$SE   <- sqrt(  diag(Xp %*% vcov(M1a)$cond %*% t(Xp))  )
MyData$SeUp <- exp(MyData$eta + 1.96 * MyData$SE) / (1 + exp(MyData$eta + 1.96 * MyData$SE)) 
MyData$SeLo <- exp(MyData$eta - 1.96 * MyData$SE) / (1 + exp(MyData$eta - 1.96 * MyData$SE))
...
#* Subsection 18.1: Model formulation----
#' A zero-inflated binomial (ZIB)  model is given by the following expression.
...
#* Subsection 18.2: Execute the ZIB GLMM----

M2a <- glmmTMB(cbind(count,Failure) ~ ftemp*ffun,
              family = "binomial",
              ziformula =~ 1,
              data = s2)
...
glmmTMB:::Anova.glmmTMB(M2a, test.statistic = c("Chisq"))
#' both results are consistent

tempMeans <- emmeans(M2a, "ftemp", data = s2)
tempMeans
pairs(tempMeans, comparisons = TRUE)
funMeans <- emmeans(M2a, "ffun", data = s2)
funMeans
pairs(funMeans, comparisons = TRUE)
...
#* Subsection 18.3 Assess overdispersion----  

#' Get the fitted values
F1 <- fitted(M2a)

#' Get the Pearson residuals
E1 <- resid(M2a, type = "pearson")
...
#' Calculate Pi (probability of a false zero).
#' Get the estimated regression part for the binary part.
gamma  <- summary(M2a)$coefficients$zi[1,"Estimate"] 
...
#' Assess overdispersion.
Npar <- length(fixef(M2a)$cond) + length(fixef(M2a)$zi) +1
...
# Section 19: Model Validation ZIB GLMM----     



#* Subsection 19.1: Plot residuals vs fitted values and covariates----
...
#' What does DHARMa tell us?
E2BINqr <- simulateResiduals(fittedModel = M2a, plot = FALSE)
plotQQunif(E2BINqr, testUniformity = TRUE, 
           testOutliers = TRUE, testDispersion = TRUE)
#' better than the binomial GLMM!
...
plotResiduals(E2BINqr, form = s2$ffun, xlab = "Rank-transformed fun") 
plotResiduals(E2BINqr, form = s2$ftemp, xlab = "Rank-transformed temp") 
#still acceptable as other parts of DHARMS still look good!

#* Subsection 19.2: Simulation for zero-inflation and one-inflation----

testZeroInflation(M2a)
#' Pretty accurate and we can skip the simulate 1000 data onwards
...
# Section 20: Visualise the results of the binomial GLMM----


#* Subsection 20.1: Visualise the model fit----
...
#' SE of fitted values are given by the square root of
#' the diagonal elements of: X * cov(betas) * t(X)
MyData$SE   <- sqrt(  diag(Xp %*% vcov(M2a)$cond %*% t(Xp))  )
MyData$SeUp <- exp(MyData$eta + 1.96 * MyData$SE) / (1 + exp(MyData$eta + 1.96 * MyData$SE)) 
MyData$SeLo <- exp(MyData$eta - 1.96 * MyData$SE) / (1 + exp(MyData$eta - 1.96 * MyData$SE))
...
#' interval. 
#' The right bottom (RT*FUN) has the highest value and largest variations
#' In comparison of both temperatures, we found higher probability 
#' of seed germinated (success) in RT. 
#' With the interaction along with temperature and pop and fun, 
#' we found higher probability of seed germination in RT * MX with 
...
#
AIC(M2, M2a)
#M2 has a lower AIC overall although pop has no significantly differences